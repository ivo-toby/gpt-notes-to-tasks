...
class ChunkingService:
    ...
    def _process_chunk_group(self, content: str, doc_type: str = 'note',
...
Return only the JSON array with no additional text."""
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to parse LLM response as JSON: {response_content[:100]}...")
                    raise

                # Validate the structure of each chunk
                valid_chunks = []
                for chunk in chunks_data:
                    if isinstance(chunk, dict) and 'content' in chunk and 'title' in chunk:
                        chunk_content = chunk['content']
                        if self.min_chunk_size <= len(chunk_content) <= self.max_chunk_size:
                            valid_chunks.append(chunk)
                        else:
                            logger.warning(f"Chunk size {len(chunk_content)} outside bounds for title: {chunk['title']}")
